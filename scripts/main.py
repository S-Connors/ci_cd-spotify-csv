...
def get_artist_albums(headers):
    ...
    resp = requests.get(os.getenv('base_url') + 'artists/' + os.getenv('artist_id') + '/albums', headers=headers)
...
def get_album_tacks(headers, artist_albums):
    ...
    for album in artist_albums['items']:
        ...
        r = requests.get(os.getenv('base_url') + 'albums/' + album['id'] + '/tracks',
                            headers=headers)
        tracks = r.json()['items']

        # get all audio features for every tack id
        for track in tracks:
            f = requests.get(os.getenv('base_url') + 'audio-features/' + track['id'], headers=headers)
            f = f.json()
